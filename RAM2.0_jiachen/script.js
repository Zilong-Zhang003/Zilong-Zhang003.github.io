...
function openHuggingFace() {
  // 跳转到Hugging Face页面
  window.open(
    "https://huggingface.co/spaces/fjc2005/DetectAnyLLM-demo",
    "_blank"
  );
...
function handleResultCardClick() {
  ...
  const messages = {
    en: {
      indoor:
        "🏠 Indoor Depth Estimation: High-precision 3D perception in complex indoor environments",
      outdoor:
        "🌳 Outdoor Depth Estimation: Adapts to various outdoor lighting and weather conditions",
      night:
        "🌙 Night Depth Estimation: Stable performance under extremely low lighting conditions",
      weather:
        "⛈️ Adverse Weather: Robust performance under extreme weather conditions like rain, snow, and fog",
    },
    zh: {
  ...
  const message =
    messages[currentLanguage][resultType] || "Experimental result details";
...
function handleResourceClick(e) {
  ...
  const link = resourceCard.querySelector("a")
    ? resourceCard.querySelector("a").href
    : null;
  ...
  const message = messages[title]
    ? messages[title][currentLanguage]
    : "Opening link...";
...
function preloadCriticalImages() {
  const criticalImages = ["image/teaser.png"];
...
function initializeVersionCarousel() {
  ...
  function updateActiveStates(version) {
    ...
    indicators.forEach((indicator) => {
      indicator.classList.toggle(
        "active",
        indicator.dataset.version === version
      );
    });
  ...
  function initializePageSliders(version) {
    const activePage = document.querySelector(
      `.carousel-page[data-page="${version}"]`
    );
  ...
  // 预加载V2和DepthPro页面的图片
  setTimeout(() => {
    const v2Images = document.querySelectorAll(
      '.carousel-page[data-page="v2"] .lazy-load:not(.loaded)'
    );
  ...
  setTimeout(() => {
    const proImages = document.querySelectorAll(
      '.carousel-page[data-page="pro"] .lazy-load:not(.loaded)'
    );
...
function initializeVisualComparison() {
  ...
  let currentTask = "blur";
  let currentMethod1 = "LQ";
  let currentMethod2 = "LQ";
  let isDragging = false;
  ...
  function updateImages() {
    const basePath = `images/vis/${currentTask}/`;

    if (image1) {
      image1.src = `${basePath}${currentMethod1}.png`;
      image1.alt = `${currentMethod1} - ${currentTask}`;
    }

    if (image2) {
      image2.src = `${basePath}${currentMethod2}.png`;
      image2.alt = `${currentMethod2} - ${currentTask}`;
    }
  }
  ...
  function updateSliderPosition(percentage) {
    ...
    if (image2Container) {
      image2Container.style.clipPath = `polygon(${percentage}% 0%, 100% 0%, 100% 100%, ${percentage}% 100%)`;
    }
  }
